...
const CustomerDashboard: React.FC<CustomerDashboardProps> = ({ user, parcel, onLogout }) => {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-background">
            <header className="bg-surface shadow-sm px-4 sm:px-6 py-3 flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-2 z-10 border-b border-border sticky top-0">
                ...
                <div className="flex items-center gap-4"><span className="text-content-secondary hidden sm:inline">Welcome, {user.name}</span><ThemeToggle /><Button onClick={onLogout} variant="secondary" className="flex items-center gap-2" aria-label="Logout"><LogoutIcon className="w-5 h-5"/><span className="hidden sm:inline">Logout</span></Button></div>
            </header>
            <main className="flex-1 p-4 sm:p-6 overflow-y-auto">
                <Card className="max-w-4xl mx-auto">
                    <div className="text-center border-b border-border pb-4 mb-4">
                        <p className="text-content-secondary">Tracking Number</p>
                        <h1 className="text-3xl font-bold font-mono text-primary">{parcel.trackingNumber}</h1>
                        <div className={`mt-2 inline-flex items-center gap-2 text-lg font-semibold ${currentStatusColor}`}><CurrentStatusIcon className="w-6 h-6" /><span>{parcel.status}</span></div>
                    </div>
                    <div className="mb-6">{renderTimeline(parcel)}</div>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <Card><h3 className="text-lg font-semibold mb-3 text-content-primary flex items-center gap-2"><MapPinIcon className="w-5 h-5 text-primary"/> Delivery Progress</h3><LahoreMap parcel={parcel} driver={deliveryDriver} /></Card>
                        <div className="space-y-4">
                            ...
                            <Card><h3 className="text-lg font-semibold mb-2 text-content-primary flex items-center gap-2"><DollarSignIcon className="w-5 h-5 text-primary"/> Amount to Pay</h3><p className="text-2xl font-bold text-green-600 dark:text-green-400">PKR {parcel.codAmount.toFixed(2)}</p><p className="text-xs text-content-muted">Please pay this amount to the driver upon delivery.</p></Card>
                        </div>
                    </div>
                </Card>
            </main>
