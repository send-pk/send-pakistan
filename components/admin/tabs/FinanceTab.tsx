...
export const FinanceTab: React.FC<FinanceTabProps> = ({ parcelsForDateRange, allParcels, users }) => {
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-bold mb-4 text-content-primary">Finances</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
               <Card>
                    <h2 className="text-xl font-bold mb-4">Ready for Payout</h2>
                    {readyForPayout.length === 0 ? <p className="text-center text-content-muted py-8">No brands have parcels ready for payout.</p> :
                    <div className="space-y-4">
                        {readyForPayout.map(payout => (
                            <div key={payout.brandId} className="p-4 bg-background rounded-lg border border-border">
                                ...
                                <div className="mt-3 pt-3 border-t border-border grid grid-cols-3 text-center text-sm">
                 ...
                 <Card>
                    ...
                    {generatedPayouts.length === 0 ? <p className="text-center text-content-muted py-8">No invoices generated yet.</p> :
                     <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-border bg-surface/50 text-content-secondary"><tr><th className="p-4 font-semibold">Brand</th><th className="p-4 font-semibold">Date</th><th className="p-4 font-semibold text-right">Amount</th><th className="p-4 font-semibold text-center">Status</th><th className="p-4 font-semibold text-center">Actions</th></tr></thead>
                            <tbody className="text-content-primary">{generatedPayouts.map(inv => (<tr key={inv.id} className="border-b border-border last:border-b-0"><td className="p-4 font-semibold">{inv.brandName}</td><td className="p-4">{formatDatePKT(inv.generatedAt)}</td><td className="p-4 text-right font-semibold">PKR {inv.netPayout.toLocaleString()}</td><td className="p-4 text-center">{inv.status === 'PENDING' ? (<Button size="sm" variant="secondary" onClick={() => setPayingInvoice(inv)}>Pay</Button>) : (<span className="inline-flex items-center gap-1.5 px-2 py-0.5 text-xs font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-500/20 dark:text-green-300">Paid</span>)}</td><td className="p-4 text-center"><Button size="sm" variant="secondary" onClick={() => setDownloadingInvoice(inv)} aria-label="Download Invoice"><DownloadIcon className="w-4 h-4" /></Button></td></tr>))}</tbody></table></div>}
                </Card>
            </div>

             <Card>
                <h2 className="text-xl font-bold mb-4">Driver COD Reconciliation</h2>
                 <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-border bg-surface/50 text-content-secondary"><tr><th className="p-4 font-semibold">Driver</th><th className="p-4 font-semibold text-right">Delivered</th><th className="p-4 font-semibold text-right">COD Handled</th><th className="p-4 font-semibold text-right">Pending Recon</th><th className="p-4"></th></tr></thead>
                        <tbody className="text-content-primary">{driverSummary.map(d => (<tr key={d.driverId} className="border-b border-border last:border-b-0"><td className="p-4 font-semibold">{d.driverName}</td><td className="p-4 text-right">{d.deliveredCount}</td><td className="p-4 text-right">PKR {d.totalCODHandled.toLocaleString()}</td><td className="p-4 text-right font-bold text-red-500">PKR {d.codPending.toLocaleString()}</td><td className="p-4 text-center">{d.codPending > 0 && <Button size="sm" variant="secondary" onClick={() => { setReconDriver(drivers.find(dr => dr.id === d.driverId) || null); setIsReconModalOpen(true); }}>Reconcile</Button>}</td></tr>))}</tbody></table></div>
            </Card>

            <Modal isOpen={!!payingInvoice} onClose={() => setPayingInvoice(null)} title={`Pay Invoice #${payingInvoice?.id.slice(-6)}`}>
                <form onSubmit={handlePaymentSubmit}><p className="mb-4">You are about to mark the invoice for <strong>{payingInvoice?.brandName}</strong> amounting to <strong>PKR {payingInvoice?.netPayout.toFixed(2)}</strong> as paid.</p><label htmlFor="transactionId" className="block mb-2 text-sm text-content-secondary font-medium">Transaction ID / Reference</label><input id="transactionId" value={transactionId} onChange={e => setTransactionId(e.target.value)} required autoFocus className="w-full bg-surface border border-border rounded-md px-3 py-2 text-content-primary focus:border-primary focus:ring-1 focus:ring-primary transition-colors" /><div className="flex justify-end gap-2 mt-6"><Button type="button" variant="secondary" onClick={() => setPayingInvoice(null)}>Cancel</Button><Button type="submit">Mark as Paid</Button></div></form>
            </Modal>
            ...
            <Modal isOpen={isReconModalOpen} onClose={() => { setIsReconModalOpen(false); setReconDriver(null); }} title={`Reconcile COD for ${reconDriver?.name}`} size="3xl">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div className="max-h-[40vh] overflow-y-auto space-y-2 p-2 bg-background rounded-lg border border-border">
                            {unreconciledParcelsForDriver.length > 0 ? unreconciledParcelsForDriver.map(p => (
                                <div key={p.id} className="flex items-center justify-between p-2 bg-surface rounded-md border border-border">
                    ...
                    <div>
                        ...
                        <div className="space-y-4 p-3 bg-background rounded-lg border border-border">
                             <div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="max-h-40 overflow-y-auto space-y-2 pr-2">
                                    {reconTransfers.map((transfer, index) => (
                                        <div key={index} className="flex items-end gap-2 p-2 bg-surface border border-border rounded-md">
                ...
                <div className="mt-4 pt-4 border-t border-border">
                    <div className="flex justify-between items-center mb-4 p-3 rounded-lg bg-background border border-border">
                        <div className="text-left">
                            <p className="text-sm text-content-secondary">Selected Parcels Total</p>
                            <p className="font-bold text-lg text-primary">PKR {totalSelectedReconAmount.toLocaleString()}</p>
                        </div>
                        <div className="text-center">
                            <p className={`font-bold text-lg transition-colors ${isReconAmountMatched ? 'text-green-500' : 'text-red-500'}`}>{isReconAmountMatched ? 'MATCHED' : 'NOT MATCHED'}</p>
                        </div>
                         <div className="text-right">
                            <p className="text-sm text-content-secondary">Amount Entered</p>
                            <p className={`font-bold text-lg ${isReconAmountMatched ? 'text-green-500' : 'text-red-500'}`}>PKR {totalEnteredReconAmount.toLocaleString()}</p>
                        </div>
                    </div>
